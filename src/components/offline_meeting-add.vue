  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        this.$http('user/searchAllUser', 'GET', null, true, resp => {
          let temp = [];
    ...
    loadPlaceList() {
      ...
      this.$http('meeting_room/searchFreeMeetingRoom', 'POST', data, true, resp => {
        this.placeList = resp.list;
      });
    },

    dataFormSubmit() {
      let data = this.dataForm
      data.date = dayjs(this.dataForm.date).format('YYYY-MM-DD');//把日期对象转成固定格式的字符串
      data.members = JSON.stringify(this.dataForm.members);//把数组转成json字符串
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http("meeting/insert", "POST", data, true,  resp => {
            if (resp.rows == 1) {
              this.visible = false
              this.$message({
                message: '操作成功',
